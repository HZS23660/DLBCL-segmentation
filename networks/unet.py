...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels1, in_channels2, out_channels, dropout_p,
        ...
        self.attention = AttentionBlock(in_channels2, in_channels1, int(in_channels1 / 2))
        if bilinear:
    ...
    def forward(self, x1, x2, Attention = False):
        if Attention:
            x_attention = self.attention(x2, x1)
        else:
            x_attention = x2
        if self.bilinear:
            x1 = self.conv1x1(x1)
        x1 = self.up(x1)
        x = torch.cat([x_attention, x1], dim=1)
        return self.conv(x)
...
class Decoder(nn.Module):
    ...
    def forward(self, feature):
        ...
        x = self.up1(x4, x3, Attention = self.attention)
        x = self.up2(x, x2, Attention = self.attention)
        x = self.up3(x, x1, Attention = self.attention)
        x = self.up4(x, x0, Attention = self.attention)
